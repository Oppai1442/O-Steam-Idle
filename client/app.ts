...
const els = {
  connection: $('#connection'), runtimeMode: $('#runtimeMode'), accountTitle: $('#accountTitle'), authText: $('#authText'),
  stateText: $('#stateText'), sessionText: $('#sessionText'), reconnectCount: $('#reconnectCount'),
  qrBtn: $('#qrBtn'), savedBtn: $('#savedBtn'), logoutBtn: $('#logoutBtn'), qrWrap: $('#qrWrap'), qr: $('#qr'), qrHint: $('#qrHint'),
...
function sourceTags(g: ClientGame): string[] {
  const tags = [];
...
function renderStatus() {
  ...
  if (yielding) connectionLabel = status.externalPlayingApp ? `Yielding · ${status.externalPlayingApp}` : 'Yielding';
  ...
  els.accountTitle.textContent = connected
  ...
  els.authText.textContent = cloud
    ? 'Cloud mode uses the configured Steam refresh-token secret. QR login works for this container session, but update the secret for durable restarts.'
    : 'Authenticate once with Steam Mobile QR. The refresh token is protected with Windows DPAPI on this PC.';
  ...
  if (status.qrDataUrl) {
    ...
    els.qrHint.textContent = status.qrStatus === 'scanned' ? 'Scanned · approve in Steam Mobile' : 'Scan with Steam Mobile';
  } else {
...
els.qrBtn.addEventListener('click', async () => {
  try { await api('/api/login/qr', {method:'POST'}); await poll(); } catch (e) { showError(e); }
});
els.savedBtn.addEventListener('click', async () => {
  try { await api('/api/login/saved', {method:'POST'}); await poll(); } catch (e) { showError(e); }
});
els.logoutBtn.addEventListener('click', async () => {
  try { await api('/api/logout', {method:'POST'}); selected.clear(); initializedSelection = false; library = []; await poll(); } catch (e) { showError(e); }
});
els.refreshBtn.addEventListener('click', async () => {
  try { await api('/api/library/refresh', {method:'POST'}); lastLibraryCount = -1; await loadLibrary(); await poll(); } catch (e) { showError(e); }
});
els.refreshCardsBtn.addEventListener('click', async () => {
  try { await api('/api/cards/refresh', {method:'POST'}); await loadLibrary(); await poll(); } catch (e) { showError(e); }
});
els.startBtn.addEventListener('click', async () => {
  try { await api('/api/idle/start', {method:'POST', body:JSON.stringify({appids:[...selected]})}); await poll(); } catch (e) { showError(e); }
});
els.stopBtn.addEventListener('click', async () => {
  try { await api('/api/idle/stop', {method:'POST'}); await poll(); } catch (e) { showError(e); }
});
els.exitBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // A connection reset here can simply mean Node exited before fetch finished.
  }
